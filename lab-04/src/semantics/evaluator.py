...
class TypedLambdaEvaluator:
    ...
    def single_step(self, state_before: Term) -> Transition:
        ...
        match state_before:
            ...
            case TmLet(fi, var, rvalue, body) if not term_is_val(rvalue):
                reduced_rvalue = self.single_step(rvalue)
                reduced_let = TmLet(fi, var, reduced_rvalue.new_state.term, body)
                return transition(reduced_let, EvalRule.Let, (reduced_rvalue,))
            case TmFix(_, function) if not term_is_val(function):
                reduced_function = self.single_step(function)
                new_fix = TmFix(Info.dummy_info(), reduced_function.new_state.term)
                return transition(new_fix, EvalRule.Fix, (reduced_function,))
            case TmFix(_, TmAbs(_, var, var_type, body)):
                new_body = term_substitute(TmFix(Info.dummy_info(), TmAbs(Info.dummy_info(), var, var_type, body)), body)
                return transition(new_body, EvalRule.FixBeta)
