...
class TypedLambdaEvaluator:
    ...
    def single_step(self, state_before: LambdaProgramState) -> Transition:
        ...
        match state_before.term:
            case TmReference(_, term) if term_is_val(term) and term not in state_before.memory.space:
                new_memory, location = state_before.memory.put(term)
                return transition(LambdaProgramState(location, new_memory), EvalRule.RefV)
            case TmReference(info, term) if not term_is_val(term):
                witness = self.single_step(updated_state(term))
                new_term = TmReference(info, witness.new_state.term)
                new_state = LambdaProgramState(new_term, witness.new_state.memory)
                return transition(new_state, EvalRule.Ref, (witness,))
            case TmDereference(info, term) if not term_is_val(term):
                witness = self.single_step(updated_state(term))
                new_term = TmDereference(info, witness.new_state.term)
                new_state = LambdaProgramState(new_term, witness.new_state.memory)
                return transition(new_state, EvalRule.Deref, (witness,))
            case TmDereference(_, term):
                value = state_before.memory.dereference(term)
                return transition(value, EvalRule.DerefLoc)
            case TmAssignment(_, left_side, right_side) if term_is_val(right_side) and term_is_val(left_side):
                new_memory = state_before.memory.replace(left_side, right_side)
                return transition(LambdaProgramState(TmUnit(Info.dummy_info()), new_memory), EvalRule.Assign)
            case TmAssignment(info, left_side, right_side) if term_is_val(right_side) and not term_is_val(left_side):
                witness = self.single_step(updated_state(left_side))
                new_term = TmAssignment(info, witness.new_state.term, right_side)
                new_state = LambdaProgramState(new_term, witness.new_state.memory)
                return transition(new_state, EvalRule.Assign1, (witness,))
            case TmAssignment(info, left_side, right_side) if not term_is_val(right_side) and term_is_val(left_side):
                witness = self.single_step(updated_state(right_side))
                new_term = TmAssignment(info, left_side, witness.new_state.term)
                new_state = LambdaProgramState(new_term, witness.new_state.memory)
